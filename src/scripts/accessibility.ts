...
export default class Accessibility {
  private logger = new EngridLogger("WWF Accessibility", "black", "pink", "👁️‍🗨️");
  ...
  private otherAmountTabSelect() {
    const otherAmountField = ENGrid.getField(
  ...
  private universalOptIns() {
    const universalOptInBlock = document.querySelector(
      ".universal-opt-in-copy, .general-opt-in-copy, .be-a-part-of-our-community"
    ) as HTMLElement;
    const universalOptInInput = document.querySelector(
      ".universal-opt-in .en__field"
    ) as HTMLElement;
    if (universalOptInBlock && universalOptInInput) {
      const label = universalOptInBlock.querySelector(
        ".label p, .label"
      ) as HTMLElement;
      if (label) {
        label.setAttribute(
        ...
        this.logger.log(
          "Added aria-labelledby to universal opt-in checkbox with label id: " +
            label.id
        );
      }
    }
  ...
  private multistepStepper() {
    ...
    this.logger.log(
      `Found ${multistepSteppers.length} multistep-stepper elements`
    );
    multistepSteppers.forEach((stepper, index) => {
      ...
      const steps = stepper.querySelectorAll(
        ".multistep-stepper__step"
      ) as NodeListOf<HTMLElement>;
      steps.forEach((step, stepIndex) => {
        const isActive = step.classList.contains(
          "multistep-stepper__step--active"
        );
        ...
        if (label) {
          label.setAttribute(
            "id",
            `multistep-step-label-${index}-${stepIndex}`
          );
  ...
  private otCookieSettings() {
    const otCookies = document.querySelector(
  ...
  private thankYouTitle() {
    if (ENGrid.isThankYouPage() && ENGrid.getPageNumber() > 1) {
      ...
      if (document.title.includes("Thank You")) {
        this.logger.log(
          'Document title already contains "Thank You", skipping update'
        );
        return;
      }
  ...
  private relocateSupporterHubAlerts() {
    if (
      ENGrid.getPageType() === "SUPPORTERHUB" &&
      ...
      if (emailField && alerts) {
        ...
        this.logger.log(
          "Relocated supporter hub alerts to be after the email field"
        );
      }
    }
  }
